...
**Update — repeatability finding on T8 (2026-08-11, same day):** this run's T8 result (correct zero-diff `BLOCKED`) turned out not to be representative. Two more runs of this exact scenario (same spec, same fixture) were dispatched as part of `tests/comparison-run-results.md`'s full 3x repeatability pass; both of the additional runs violated the module-boundary rule this run correctly upheld — importing and calling `core._invalidate_price_cache()` directly, the same construct Condition A (no spec loaded) produced deterministically. See `tests/comparison-run-results.md`'s "Repeatability check on all 12 Haiku 4.5 Condition B pairings" section for the full detail. This run's T8 entry above should be read as 1 of 3 outcomes, not the representative one — the module boundary in `agent/icm-minimal-change-engineer.md`/`CONTEXT.md` holds against this specific failure only 1/3 of the time on this model, not reliably.
