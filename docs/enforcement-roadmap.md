...
**Task files are part of the enforced surface (F13).** The Stop hook passes `ICM_TASK_FILE` to the gate as a *path*, not only as parsed contents, so the gate can detect a session that edited its own declarations; and `hooks/protected-paths.txt` ships with `tasks/` listed, so the PreToolUse hook refuses the edit before it happens. Both matter because the earlier arrangement was self-defeating: a session could widen its own in-repo task file to `allowed_paths: [.]` and the Stop hook would report the resulting PASS to the model as automated verification. The pre-hoc block is the stronger of the two — the gate's default mode is blind to committed work, so an agent that commits a widened task file before handoff still evades the after-the-fact check unless the gate is run with `--base`, which is the same `--base` gap noted above rather than a new one.

**Pre-emptive per-task scope (2026-08-14).** `protect_governing_files.py` now also reads `ICM_TASK_FILE` and refuses a write outside that task's `allowed_paths` before it happens. This closes the asymmetry the two hooks previously had: the standing protected list was enforced up front, while per-task scope was audited only at handoff by the Stop hook, so a session could do an hour of unauthorized work and learn about it at the end. The Stop hook remains, and remains necessary — it audits the real Git diff, including changes made by anything other than the Write/Edit tools, which a PreToolUse hook never sees. The two are complementary, not redundant: one prevents the common case, the other catches what prevention cannot see. Editing the active task file is denied outright, which is the pre-emptive half of F13. Opt-in through the same `ICM_TASK_FILE` convention, so a session that does not set it gets exactly the previous behavior.

**Known limits, stated plainly:** both hooks are Claude-Code-specific (the "other environments" install path in the README has no equivalent yet). The Stop hook's `ICM_TASK_FILE` convention is a single global pointer per session — it does not support multiple concurrently-tracked tasks, and has no way to audit committed work (see above). Both hooks now have an automated suite, `tests/hooks_selftest.sh`, which generalizes the hook-input pipe-test method used during development (real stdin payloads piped directly into each script) into repeatable assertions on the emitted decision contract, and was itself negative-tested by mutating each hook. A live end-to-end fire test (wiring the PreToolUse hook into `.claude/settings.local.json` and triggering a real Write) was also attempted once and did not observe the hook firing — the settings watcher only picks up hook config from directories that already had a settings file when the session started, so a config file created mid-session needs a `/hooks` reload or restart, which the pipe-tests already prove is not a logic defect. Both are optional and off by default — adopting them is a per-project choice, made by editing `.claude/settings.json`, not something this package does for you.
