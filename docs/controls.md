...
- **Live per-task scope block** (`hooks/protect_governing_files.py`, PreToolUse, opt-in via `ICM_TASK_FILE`): denies a write outside the task's `allowed_paths` at the moment it is attempted, rather than reporting it at handoff. Until this shipped, the only pre-emptive control was the standing protected-path list, which is task-independent; per-task scope was audited after the work was done, so an agent could edit a dozen unauthorized files and be told at the end. Editing the active task file is denied as self-authorization and outranks the protected-file message, mirroring the gate's exit 4 > 2 > 1 precedence. Reuses the gate's own `normalize()`/`match_any()`, so there is one matching implementation. Fails **open** on an unreadable or malformed task file, saying so on stderr, matching this hook's existing posture: a broken control must degrade visibly to "not enforced", never silently to "everything blocked". (Shipped and self-tested, mutation-tested three ways.)
