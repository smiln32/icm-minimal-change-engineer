...
### Added
- **The PreToolUse hook now enforces per-task scope live, before the write.** Previously the only pre-emptive control was the standing protected-path list; per-task `allowed_paths` was audited by the Stop hook at handoff, so an agent could edit a dozen unauthorized files and be told about it once the work was done. With `ICM_TASK_FILE` set, a write outside `allowed_paths` is now denied at the moment it is attempted, and the denial names both the offending path and what the task actually authorizes. Editing the active task file is denied as self-authorization and outranks the protected-file message, mirroring the gate's exit 4 > 2 > 1 precedence — the pre-emptive half of F13. Reuses the gate's own `normalize()`/`match_any()`, so there is one matching implementation rather than two that could drift. Fails **open** on an unreadable or malformed task file, announcing on stderr that scope enforcement is not active, matching this hook's existing posture. With `ICM_TASK_FILE` unset the hook has no scope opinion at all, so non-ICM sessions in a project with the hooks installed behave exactly as before. Hooks self-test cases 21-24; mutation-tested three ways — removing the scope deny fails 4 assertions, dropping the task-file precedence fails 2, and breaking the opt-in so it denies by default fails 5. The Stop hook is unchanged and still necessary: it audits the real Git diff, including changes made by anything other than the Write/Edit tools, which a PreToolUse hook cannot see.
- **`install.py` — installing into a real project is now one command.** Adopting the package previously meant seven manual steps, four of which were not written down anywhere: copying the agent definition, copying `hooks/` *and* `tests/scope_gate.py` while preserving their relative layout, editing `protected-paths.txt` for your project rather than this one, merging the settings snippet, writing a task file, setting `ICM_TASK_FILE`, and restarting Claude Code. `python3 install.py /path/to/project` does the mechanical four, with `--dry-run` and `--no-hooks`. Everything lands in `.claude/` and a single `.icm/` directory, never the project root: the old instructions would have created `hooks/` and `tests/` directories in a project that almost certainly has a `tests/` of its own. Existing hooks in `.claude/settings.json` are preserved rather than overwritten, and the protected-paths list is generated from the governing files the target project actually contains, with the rest written commented out so the file doubles as a menu.
- Both hooks now locate the gate across layouts (`ICM_GATE`, the package's `../tests/`, or the installed `../scope_gate.py`), so the same files work in this repository and in an installed project with no edit at install time.

### Fixed
- `install.py` writes `tasks/` into the generated protected-paths list unconditionally. It was being commented out because the scan runs before the installer creates the directory, which would have shipped every new installation with the F13 hole open: task files declare the scope the gate enforces, so an agent able to edit one widens its own permissions and passes its own audit.
- The PreToolUse deny message named `hooks/protected-paths.txt` literally. In an installed project that path does not exist — the file is at `.icm/hooks/protected-paths.txt` — so the message sent the reader somewhere they could not act on. It now reports the config relative to the project directory when it sits inside it, absolute otherwise.

- **Reporting a run is now one command.** `make_fixture.py --report <fixture> --model ... --condition A|B` writes an `icm-result-<scenario>-<condition>.md` carrying everything a machine can determine: the real scope-gate exit code and output, the real changed-file list, whether anything was written under `approvals/`, `decisions/` or `tasks/`, the platform, and the package version and commit that built the fixture. Two fields are left for the tester, the status the agent reported and any notes, because neither is reliably readable out of a transcript. `--model` and `--condition` are required rather than defaulted: a result recording neither cannot be compared with anything, and a default would let one be produced by accident. The fixture is stamped inside `.git/` rather than the working tree, so the stamp is invisible to the agent and to every diff the gate audits.
- Each scenario's `PROMPT.md` is rewritten as numbered steps — build, set up the agent, send this exact message, send the second message where there is one, generate the report, send it — with the trap explanation moved below the steps rather than mixed into them.
- `tests/results/` for filing returned result files, with the reasoning for keeping them as files carrying their own provenance rather than flattening them into a summary table, and a warning against pooling across models.
