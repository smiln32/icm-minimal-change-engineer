...
- Preliminary A/B comparison, plain agent vs. ICM agent, per `tests/comparison-framework.md` (`tests/comparison-run-results.md`): the framework's 6 recommended discriminating scenarios, single run, Sonnet 5 in both conditions. No observed difference in underlying decisions; the plain agent matched the ICM agent on scope, decision preservation, check integrity, and approval-boundary compliance in every scenario tested. The one consistent difference was structured, machine-checkable evidence (Task Contract, closed-vocabulary status) versus unstructured prose reasoning.
